/**
 * Douyin API Audio Extraction Test Script (Local Node.js Version)
 */

const TEST_CONFIG = {
    // 替换为真实的主页链接或视频链接进行测试
    testUrl: 'https://www.douyin.com/user/MS4wLjABAAAA0hbVQK7dw1cuCT1CyRXZ-5EYnHqgwnuC4sJz8nvypnc?from_tab_name=main&relation=0&vid=7601901013892746532',

    // 通用混淆参数 (来自 [请求头.txt] 和 Python 稳定版)
    ...
    // 列表 API 基础
    postBase: "https://www.douyin.com/aweme/v1/web/aweme/post/?device_platform=webapp&aid=6383&channel=channel_pc_web&pc_client_type=1",

    // 详情 API 基础
    detailBase: "https://www.douyin.com/aweme/v1/web/aweme/detail/?device_platform=webapp&aid=6383&channel=channel_pc_web&pc_client_type=1",
...
async function testExtraction() {
    ...
    try {
        ...
        if (targetUrl.includes('/video/')) {
            ...
            if (awemeId) {
                console.log("🎬 识别到视频 ID:", awemeId);
                await fetchVideoDetail(awemeId);
            }
        } else {
            const secUid = targetUrl.match(/\/user\/([a-zA-Z0-9_-]+)/)?.[1] ||
                new URL(targetUrl).searchParams.get('sec_user_id');
            if (secUid) {
                console.log("👤 识别到用户 SecUID:", secUid);
                await fetchUserVideos(secUid);
            } else {
                console.error("❌ 无法提取有效 ID");
            }
        }
    } catch (e) {
        console.error("⚠️ 预检出错:", e.message);
    }
}

async function fetchVideoDetail(awemeId) {
    const apiUrl = TEST_CONFIG.detailBase + `&aweme_id=${awemeId}` + TEST_CONFIG.commonParams;
    console.log("🌐 请求详情 API:", apiUrl.substring(0, 150) + "...");

    try {
        const response = await fetch(apiUrl, { headers: TEST_CONFIG.headers });
        console.log(`📡 响应状态: ${response.status} ${response.statusText}`);

        const text = await response.text();
        if (!text) {
            console.error("❌ 响应正文为空");
            return;
        }
        ...
        try {
            ...
            if (aweme) {
                parseAndLogVideo(aweme);
            } else {
                console.error("❌ API 未返回 aweme_detail, 完整响应:", text.substring(0, 500));
            }
        } catch (jsonErr) {
            console.error("❌ JSON 解析失败:", jsonErr.message);
            console.log("📄 原始响应内容 (前500字):", text.substring(0, 500));
        }
    } catch (e) {
...
async function fetchUserVideos(secUid) {
    const apiUrl = TEST_CONFIG.postBase + `&sec_user_id=${secUid}&max_cursor=0&count=1` + TEST_CONFIG.commonParams;
    console.log("🌐 请求列表 API (仅获取 1 个):", apiUrl.substring(0, 150) + "...");
    ...
    try {
        ...
        if (data.aweme_list?.length > 0) {
            console.log(`✅ 成功获取 ${data.aweme_list.length} 个视频`);
            parseAndLogVideo(data.aweme_list[0]);
        } else {
            console.warn("⚠️ 列表为空, 原始响应:", text.substring(0, 500));
        }
    } catch (e) {
        console.error("❌ 列表请求失败:", e.message);
    }
}

function parseAndLogVideo(aweme) {
    ...
    if (video.bit_rate && video.bit_rate.length > 0) {
        // 遍历所有码率分支，寻找包含 "media-audio-und-mp4" 的 URL
    ...
    console.log("🎬 视频ID:", aweme.aweme_id);
    if (audioUrl) {
        console.log("📡 目标音频流 (media-audio):", audioUrl);
    } else {
        console.error("❌ 未能在数据中找到符合 media-audio-und-mp4 的流地址");
        console.log("调试：可用码流列表如下:", JSON.stringify(video.bit_rate?.[0]?.play_addr?.url_list, null, 2));
    }
    console.log("-----------------------------------------");
}
